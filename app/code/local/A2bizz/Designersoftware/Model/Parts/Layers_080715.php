...
class A2bizz_Designersoftware_Model_Parts_Layers extends Mage_Core_Model_Abstract
{
	...
	public function getColorPrice($partsLayersId, $colorId){
		...
		if(!empty($partsLayersId)):
			...
			if(empty($collection->getData('color_price'))):				
				if($collection->getData('default_color_price')>0):
					return $collection->getData('default_color_price');
				else:	
					return 0;
				endif;
			else:
