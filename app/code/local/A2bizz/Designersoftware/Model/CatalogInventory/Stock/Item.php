 ...
 */
class A2bizz_Designersoftware_Model_CatalogInventory_Stock_Item extends Mage_CatalogInventory_Model_Stock_Item
{
            ...
            if (($this->getQty() - $summaryQty) < 0) {
                if ($this->getProductName()) {
                    ...
                    if ($this->getBackorders() == Mage_CatalogInventory_Model_Stock::BACKORDERS_YES_NOTIFY) {
                        if (!$this->getIsChildItem()) {
							$message = Mage::getStoreConfig('cataloginventory/item_options/message');
							if(!empty($message)):								
								$result->setMessage(Mage::getStoreConfig('cataloginventory/item_options/message'));
							else:
							$result->setMessage(
