...
class A2bizz_Designersoftware_Helper_Data extends Mage_Core_Helper_Abstract
{	
	...
	/*public function getTexture($includeNone = false){
		$textureArr=array();
		
		if ($includeNone){
			$textureArr['']='--- Please Select ---';
		}
	...
	public function getSizesColors($sizes_id, $includeNone = false){
		$sizesArr=array();
		...
		$collection = Mage::getModel('designersoftware/sizes')->getCollection()
			->addFieldToFilter('sizes_id',$sizes_id)
			->addFieldToFilter('status',array('eq'=>1))
			->getFirstItem();
		...
		if($collection->getId()>0){
			...
			foreach($colorIdArray as $value){
				...
				$sizesArr[$collection->getColorcode()]=$collection->getTitle();								
			}
