<?php

class A2bizz_Designersoftware_Block_Adminhtml_Texture_Edit_Tabs extends Mage_Adminhtml_Block_Widget_Tabs
{

  public function __construct()
  {
      parent::__construct();
      $this->setId('texture_tabs');
      $this->setDestElementId('edit_form');
      $this->setTitle(Mage::helper('designersoftware')->__('Texture Information'));
  }

  protected function _beforeToHtml()
  {
      $this->addTab('form_section', array(
          'label'     => Mage::helper('designersoftware')->__('Texture Information'),
          'title'     => Mage::helper('designersoftware')->__('Texture Information'),
          'content'   => $this->getLayout()->createBlock('designersoftware/adminhtml_texture_edit_tab_form')->toHtml(),
      ));
     
      return parent::_beforeToHtml();
  }
}
